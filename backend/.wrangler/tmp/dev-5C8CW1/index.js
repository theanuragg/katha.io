...
// .wrangler/tmp/bundle-TJjFXA/middleware-loader.entry.ts
init_modules_watch_stub();

// .wrangler/tmp/bundle-TJjFXA/middleware-insertion-facade.js
init_modules_watch_stub();
...
userRouter.post("/signin", async (c) => {
  ...
  try {
    ...
    const token = sign2({ payload: user.id }, c.env.JWT_SECRET);
    console.log(token);
    return c.json({ token });
  } catch (e) {
    c.status(401);
